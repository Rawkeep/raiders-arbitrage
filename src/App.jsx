...
import { exportAllData, importData } from "./db.js";
import { searchListings, getApiStatus, importManualListings, importCsvListings } from "./api.js";
...
// ─── SETUP / TRAINING ───
function SetupStage({ config, setConfig, onNext, apiStatus }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      {/* Data Sources Info */}
      {apiStatus && (
        <Card glow={apiStatus.ebay || apiStatus.etsy ? "#10B981" : "#F59E0B"} style={{ borderLeft: `3px solid ${apiStatus.ebay || apiStatus.etsy ? "#10B981" : "#F59E0B"}` }}>
          <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
            <span style={{ fontSize: 18 }}>{apiStatus.ebay || apiStatus.etsy ? "🟢" : "⚠️"}</span>
            <div>
              <div style={{ color: apiStatus.ebay || apiStatus.etsy ? "#10B981" : "#F59E0B", fontWeight: 700, fontSize: 13 }}>
                {apiStatus.ebay || apiStatus.etsy ? "Live-APIs aktiv" : "Demo-Modus — APIs nicht konfiguriert"}
              </div>
              <div style={{ color: "#94a3b8", fontSize: 12, marginTop: 4, lineHeight: 1.5 }}>
                {apiStatus.ebay && "eBay API ● "}{apiStatus.etsy && "Etsy API ● "}
                {!apiStatus.ebay && !apiStatus.etsy && "Trage API-Keys in .env ein für echte Ergebnisse. "}
                Kleinanzeigen/Vinted/FB via Browser Extension oder CSV Import.
              </div>
            </div>
          </div>
        </Card>
...
// ─── SEARCH ───
function SearchStage({ listings, searching, onClassify, searchError, apiStatus }) {
  const grouped = {};
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      {apiStatus && !searching && (
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
          {[
            { key: "ebay", label: "eBay API", color: "#E53238" },
            { key: "etsy", label: "Etsy API", color: "#F56400" },
            { key: "manual", label: "Manuell/CSV", color: "#86BC25" },
          ].map(s => (
            <span key={s.key} style={{
              padding: "4px 10px", borderRadius: 6, fontSize: 11, fontWeight: 600,
              background: apiStatus[s.key] ? s.color + "15" : "#EF444415",
              color: apiStatus[s.key] ? s.color : "#EF4444",
              border: `1px solid ${apiStatus[s.key] ? s.color + "33" : "#EF444433"}`,
            }}>
              {apiStatus[s.key] ? "●" : "○"} {s.label}
            </span>
          ))}
        </div>
      )}
      ...
      {/* Error/Info Banner */}
      {searchError && (
        <Card glow="#F59E0B" style={{ borderLeft: "3px solid #F59E0B", padding: 14 }}>
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            <span style={{ fontSize: 16 }}>⚠️</span>
            <span style={{ color: "#F59E0B", fontSize: 12, lineHeight: 1.4 }}>{searchError}</span>
          </div>
        </Card>
      )}
...
// MAIN APP
// ═══════════════════════════════════════
export default function FlipFlowPro({ onLogout, session }) {
  const [stage, setStage] = usePersistedState("stage", STAGES.DASHBOARD);
  const [config, setConfig] = usePersistedState("config", {
  ...
  const [listings, setListings] = usePersistedState("listings", []);
  const [accounts, setAccounts] = usePersistedState("accounts", [
  ...
  const [apiStatus, setApiStatus] = useState({ ebay: false, etsy: false, manual: true });
  const [showMenu, setShowMenu] = useState(false);
  const importRef = useRef(null);
  const csvImportRef = useRef(null);

  // Check API status on mount
  useEffect(() => { getApiStatus().then(setApiStatus).catch(() => {}); }, []);
  ...
  const handleCsvImport = async (e) => {
    ...
    try {
      ...
      const result = await importCsvListings(text, config);
      if (result.items?.length > 0) {
        setListings(prev => [...result.items, ...prev]);
        setStage(STAGES.CLASSIFY);
      }
    } catch (err) {
  ...
  const startSearch = async () => {
    ...
    try {
      ...
      const result = await searchListings({
        ...
        sellerTypes: config.sellerTypes,
        config,
      });
      ...
      if (result.items && result.items.length > 0) {
        ...
        setListings(generateListings(config));
        setSearchError("Keine API-Ergebnisse — Demo-Daten geladen. Prüfe deine API-Keys in .env");
      }
    } catch (err) {
      // Offline fallback: generate demo data
      console.warn("API not available, using demo data:", err.message);
      setListings(generateListings(config));
      setSearchError("Backend nicht erreichbar — Demo-Modus aktiv. Starte den Server mit: npm run server");
    }
  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(180deg, #06060c 0%, #0a0a18 100%)", color: "#e2e8f0", fontFamily: "'DM Sans', system-ui, sans-serif" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div style={{ position: "relative" }}>
            <button onClick={() => setShowMenu(m => !m)} style={{
              ...
              color: "#94a3b8", padding: "4px 8px", fontSize: 14, cursor: "pointer", lineHeight: 1,
            }}>☰</button>
            {showMenu && (
              <div style={{
                position: "absolute", right: 0, top: "calc(100% + 6px)",
                background: "#12121e", border: "1px solid #ffffff12", borderRadius: 10,
                padding: 6, minWidth: 180, zIndex: 200, boxShadow: "0 8px 32px #00000088",
              }}>
                <input ref={importRef} type="file" accept=".json" onChange={handleImport}
                  style={{ display: "none" }} />
                <input ref={csvImportRef} type="file" accept=".csv,.tsv,.txt" onChange={handleCsvImport}
                  style={{ display: "none" }} />
                {session?.token && (
                  <div style={{ padding: "6px 12px", borderBottom: "1px solid #ffffff08", marginBottom: 4 }}>
                    <div style={{ color: "#475569", fontSize: 10, marginBottom: 2 }}>Token</div>
                    <div style={{ color: "#64748b", fontSize: 11, fontFamily: "'Space Mono', monospace" }}>{session.token}</div>
                  </div>
                )}
                {[
                  { label: "📥 Daten exportieren", action: () => { exportAllData(); setShowMenu(false); } },
                  { label: "📤 Daten importieren", action: () => { importRef.current?.click(); setShowMenu(false); } },
                  { label: "📄 CSV Listings importieren", action: () => { csvImportRef.current?.click(); setShowMenu(false); } },
                  ...(onLogout ? [{ label: "🚪 Abmelden", action: () => { setShowMenu(false); onLogout(); } }] : []),
                ].map(item => (
      ...
      <div style={{ maxWidth: 940, margin: "0 auto", padding: "20px 16px 60px" }}>
        ...
        {stage === STAGES.SETUP && <SetupStage config={config} setConfig={setConfig} onNext={startSearch} apiStatus={apiStatus} />}
        {stage === STAGES.SEARCH && <SearchStage listings={listings} searching={searching} onClassify={() => setStage(STAGES.CLASSIFY)} searchError={searchError} apiStatus={apiStatus} />}
